...
SELECT dblink_connect('conn_branchb', 'host=localhost dbname=Branch_B user=postgres password=1234');

SELECT *
FROM dblink( 
'host=local host dbname=Branch_B user=postgres password=1234 [port=5432]'
    'SELECT salaryid, staffid, basepay, allowances, deductions, new_salaryid FROM public.salary'
) AS remote_salary(
    salaryid INT,
    staffid INT,
...
INSERT INTO Staff(FullName, DeptID, Role, Email, HireDate)
VALUES ('Allen', 1, 'Manager', 'alen@example.com', '2025-9-24');
...
--  removing Later, rollback to simulate failure
ROLLBACK PREPARED 'fail_test';
...
WHERE staffid = 2;
---- 
--- used to check the lock and unlocked transaction
SELECT
    pid,
    locktype,
